...
import { searchContentDetails } from './geminiService';
import { searchTmdbContent } from './tmdbService';
import { addToNotionDatabase, listDatabases } from './notionService';
import { ContentMetadata, AppStatus, NotionDatabase } from './types';
...
const App: React.FC = () => {
  ...
  return (
    <div className="w-[400px] h-[350px] bg-[#E3F2FD] rounded-xl shadow-2xl overflow-hidden flex flex-col border border-white/40 ring-1 ring-black/5">
      {/* Mac Chrome Header */}
      <div className="bg-[#B9E2FE] px-3 pt-3 pb-0 flex flex-col gap-2 shrink-0">
        ...
        <div className="flex items-end gap-1 px-1 mt-1">
          ...
          >
            <Settings className="w-3 h-3" /> 설정 {(!apiKey || !selectedDbId) && <div className="w-1 h-1 bg-red-500 rounded-full" />}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 relative">
        {activeTab === 'setup' ? (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest px-1">1. Notion API Key</label>
              <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="secret_..." className="w-full px-3 py-2 bg-white/60 border border-white/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 text-xs transition-all" />
            </div>

            <div className="space-y-3">
              <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest px-1">2. Database Selection</label>
              {needsProxyDemo && (
                <a href="https://cors-anywhere.herokuapp.com/corsdemo" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-red-500 text-white rounded-lg text-[10px] font-black hover:bg-red-600 transition-colors">
                  <ShieldAlert className="w-3 h-3" /> Proxy 활성화가 필요합니다
                </a>
              )}
              {databases.length > 0 && (
              ...
              <button onClick={handleFetchDatabases} disabled={isLoadingDbs || !apiKey.trim()} className="w-full py-2 bg-blue-500 text-white rounded-lg text-xs font-bold hover:bg-blue-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            <div className="space-y-2 pt-2 border-t border-white/40">
              <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest px-1">3. TMDB Key (Optional)</label>
              <input type="password" value={tmdbKey} onChange={(e) => setApiKey(e.target.value)} placeholder="API key..." className="w-full px-3 py-2 bg-white/60 border border-white/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 text-xs" />
            </div>
          </div>
        ) : (
            ...
            <div className="space-y-2">
              ...
              {status === AppStatus.IDLE && !error && (
                <div className="py-10 text-center space-y-2 opacity-30">
                  ...
                  <p className="text-[10px] font-medium">검색을 시작하세요</p>
                </div>
              )}
            </div>
          </div>
        ...
        {error && (
          <div className="absolute bottom-4 left-4 right-4 p-2 bg-red-500 text-white rounded-lg text-[9px] font-bold flex items-center gap-2 shadow-xl animate-in fade-in slide-in-from-bottom-2">
            <AlertCircle className="w-3 h-3 shrink-0" /> <span className="flex-1">{error}</span>
...
const ImagePreview: React.FC<{ src: string }> = ({ src }) => {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <img src={src} className={`w-full h-full object-cover transition-opacity duration-300 ${loading ? 'opacity-0' : 'opacity-100'}`} onLoad={() => setLoading(false)} onError={() => setError(true)} referrerPolicy="no-referrer" />
    </div>
  );
};
